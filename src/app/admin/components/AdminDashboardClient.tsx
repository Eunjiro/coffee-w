...
export default function AdminDashboardClient() {
      ...
      <AdminLayout>
        <div className="p-4 md:p-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="bg-white rounded-xl p-4 md:p-6 shadow-md animate-pulse">
  ...
  const topSellers = data?.bestSellers?.today || [];
  const lowStockItems = data?.lowStock?.today || [];
  ...
  const revenueChange = yesterdayRevenue
    ...
    : 0;
  const revenueChangeClass = revenueChange >= 0 ? "text-green-600" : "text-red-600";
      ...
      <div className="bg-[#F3EEEA] p-8 h-full overflow-y-auto custom-scrollbar">
        ...
        <div className="gap-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 mb-8">
          ...
          <div className="bg-white shadow-sm p-6 pb-0 rounded-xl">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="font-bold text-[#776B5D] text-3xl">{formatCurrency(totalRevenueToday)}</p>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center text-sm">
                ...
                <span className={`font-medium ${revenueChangeClass}`}>
                  {revenueChangeSign}{revenueChange.toFixed(1)}% from yesterday
                </span>
              </div>
          ...
          <div className="bg-white shadow-sm p-6 rounded-xl">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="font-bold text-[#776B5D] text-3xl">{totalOrdersToday}</p>
              </div>
          ...
          <div className="bg-white shadow-sm p-6 rounded-xl">
            ...
            <div className="space-y-2">
              {topSellers.slice(1, 3).map((item: any, index: number) => (
        ...
        <div className="gap-6 grid grid-cols-1 lg:grid-cols-2 mb-8">
          {/* Weekly Sales Trend */}
          <div className="bg-white shadow-sm p-6 rounded-xl">
            <h3 className="font-semibold text-[#776B5D] text-lg mb-4">Weekly Sales Trend</h3>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={salesOverview}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#B0A695" />
                  <XAxis dataKey="date" stroke="#776B5D" />
                  <YAxis stroke="#776B5D" tickFormatter={(val) => `₱${val}`} />
                  <Tooltip formatter={(val) => formatCurrency(Array.isArray(val) ? val[0] : val)} />
                  <Area
                    type="monotone"
                    dataKey="total"
                    stroke="#776B5D"
            ...
            {recentOrders.map((order: any) => (
              <div key={order.id} className="bg-[#F3EEEA] p-4 border border-[#B0A695]/20 hover:border-[#776B5D]/30 rounded-lg transition-colors">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <p className="font-semibold text-[#776B5D] text-sm">Order #{order.id}</p>
                    <p className="text-[#776B5D]/70 text-xs">{new Date(order.createdAt).toLocaleString()}</p>
                  </div>
                ...
                <div className="space-y-2 mb-3">
                  ...
                  {order.items?.length > 2 && (
                    <div className="text-[#776B5D]/70 text-xs">+{order.items.length - 2} more items</div>
                  )}
                </div>
                <div className="flex justify-between items-center text-sm text-[#776B5D]/70">
                  <span>{order.paymentMethod || "-"}</span>
                  <span className="font-semibold text-[#776B5D]">{formatCurrency(order.total)}</span>
                </div>
              </div>
            ))}
          </div>
