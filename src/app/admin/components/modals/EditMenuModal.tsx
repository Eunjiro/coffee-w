...
interface EditMenuModalProps {
  open: boolean;
  item: MenuItem | null;
  ingredients: IngredientOption[];  // Removed cups from the props
  onClose: () => void;
...
const EditMenuModal: React.FC<EditMenuModalProps> = ({
  ...
  const handleCupChange = (size: SizeKey, field: "cupId" | "price", value: string) => {
  ...
  const handleIngredientChange = (size: SizeKey, index: number, field: "ingredientId" | "quantity", value: string) => {
    setIngredientsState((prev) => ({
      ...prev,
      [size]: prev[size].map((ing, i) =>
        i === index ? { ...ing, [field]: field === "ingredientId" ? Number(value) : value } : ing
      ),
    }));
  };
